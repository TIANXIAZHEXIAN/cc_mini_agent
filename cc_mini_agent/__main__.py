...
from cc_mini_agent.providers import PROVIDER_PRESETS
from cc_mini_agent.tools import get_default_tools
from cc_mini_agent.memory.memory import Memory
from cc_mini_agent.memory.dream import (
    DreamEngine, DreamConfig,
    create_dream_hook, create_session_record_hook,
)

from dotenv import load_dotenv
load_dotenv()  # 这行代码会把 .env 文件里的内容加载到系统环境变量中
...
def _parse_args():
    ...
    parser = argparse.ArgumentParser(
        prog="cc",
        description="cc_mini_agent — Python Agent Framework",
    )
    parser.add_argument(
        "--language", type=str, default=None,
        help="Preferred language for agent responses (e.g. japanese, chinese, spanish). "
             "Also reads from CC_MINI_AGENT_LANGUAGE env var.",
    )
    parser.add_argument(
...
async def repl():
    ...
    # 只需要这一行，以后扩展只需改 JSON
    config.load_mcp_from_file("mcp_config.json")
    ...
    console.print(Panel.fit(
        f"[bold cyan]cc_mini_agent[/bold cyan] — Python Agent Framework\n"
    ...
    console.print("[dim]Use [bold]Alt+Enter[/bold] to insert a newline. (使用 [bold]Alt+Enter[/bold] 换行)[/dim]\n")

    # === 新增：配置 prompt_toolkit 会话与快捷键 ===
    ...
    @bindings.add('escape', 'enter') # 兼容配置：Alt+Enter (或按一下 Esc 再按 Enter) 也能换行
    def newline(event):
        """按下 Shift+Enter 或 Alt+Enter 插入换行符而不提交"""
        event.current_buffer.insert_text('\n')

    # 启用 multiline 模式支持多行渲染
    ...
    try:
        while True:
            try:
                # === 修改：使用 session.prompt_async 替代 console.input ===
                # 原代码: user_input = console.input("[bold green]> [/bold green]")
