"""
CC_MINI_AGENT.md Config — 项目级指令文件系统

Discovers and loads CC_MINI_AGENT.md instruction files from multiple sources,
assembled in priority order and injected into the system prompt.

File loading hierarchy (lowest → highest priority):
  1. User instructions  (~/.cc_mini_agent/CC_MINI_AGENT.md)       — Private global instructions
  2. Project instructions (CC_MINI_AGENT.md, .cc_mini_agent/CC_MINI_AGENT.md, .cc_mini_agent/rules/*.md)
     — Checked into the codebase, discovered by traversing from CWD upward
  3. Local instructions  (CC_MINI_AGENT.local.md)          — Private project-specific, not committed
...
Architecture note / 架构说明:
  This is the cc_mini_agent equivalent of CLAUDE.md. It provides project-level
...
# ────────────────────────────────────────────────────────────────
# Process .cc_mini_agent/rules/ directory
# ────────────────────────────────────────────────────────────────
...
def _process_rules_dir(
    ...
    """Process all .md files in a .cc_mini_agent/rules/ directory (recursive)."""
...
def get_user_cc_mini_agent_dir() -> str:
    """Return the user-level config directory (~/.cc_mini_agent/)."""
    return os.path.join(os.path.expanduser("~"), ".cc_mini_agent")


def discover_instruction_files(cwd: Optional[str] = None) -> List[InstructionFile]:
    """Discover all CC_MINI_AGENT.md instruction files from all sources.

    Loading order (lowest → highest priority):
      1. User:    ~/.cc_mini_agent/CC_MINI_AGENT.md, ~/.cc_mini_agent/rules/*.md
      2. Project: CC_MINI_AGENT.md, .cc_mini_agent/CC_MINI_AGENT.md, .cc_mini_agent/rules/*.md
                  (discovered by walking from CWD upward to filesystem root)
      3. Local:   CC_MINI_AGENT.local.md (per-directory, not committed)
    ...
    user_dir = get_user_cc_mini_agent_dir()

    # ~/.cc_mini_agent/CC_MINI_AGENT.md
    user_cc_mini_agent_md = os.path.join(user_dir, "CC_MINI_AGENT.md")
    result.extend(_process_file(user_cc_mini_agent_md, "User", processed))

    # ~/.cc_mini_agent/rules/*.md
    ...
    for d in reversed(dirs):
        # CC_MINI_AGENT.md in directory root
        project_path = os.path.join(d, "CC_MINI_AGENT.md")
        result.extend(_process_file(project_path, "Project", processed))

        # .cc_mini_agent/CC_MINI_AGENT.md
        dot_cc_mini_agent_path = os.path.join(d, ".cc_mini_agent", "CC_MINI_AGENT.md")
        result.extend(_process_file(dot_cc_mini_agent_path, "Project", processed))

        # .cc_mini_agent/rules/*.md
        rules_dir = os.path.join(d, ".cc_mini_agent", "rules")
        result.extend(_process_rules_dir(rules_dir, "Project", processed))

        # CC_MINI_AGENT.local.md (local, not committed)
        local_path = os.path.join(d, "CC_MINI_AGENT.local.md")
