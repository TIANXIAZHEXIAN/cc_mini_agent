...
from cc_mini_agent.core.hooks import HookContext, HookEvent, HookManager, HookResult

from cc_mini_agent.config import Config
from cc_mini_agent.core.messages import (
...
from cc_mini_agent.core.permissions import PermissionContext, check_permission
from cc_mini_agent.providers import LLMProvider, create_provider
...
def _fill_missing_tool_results(messages: list) -> list:
    ...
    from cc_mini_agent.core.messages import ToolResult as TR
...
class Engine:
    ...
    def __init__(
        ...
        memory: Optional[Any] = None,  # cc_mini_agent.memory.Memory
        session_persistence: Optional[Any] = None,  # cc_mini_agent.memory.SessionPersistence
    ):
        self.config = config
        ...
        if self.session_persistence is not None:
            from cc_mini_agent.memory.session_persistence import create_session_persistence_hook
    ...
    def _build_system_prompt(self) -> str:
        ...
        # Auto-discover CC_MINI_AGENT.md instruction files
    ...
    async def run(
        ...
        while turns < max_turns:
            ...
            if self.config.parallel_tool_execution and len(assistant_msg.tool_calls) > 1:
                from cc_mini_agent.core.streaming_executor import execute_tools_parallel
