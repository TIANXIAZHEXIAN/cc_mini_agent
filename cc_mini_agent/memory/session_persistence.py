...
Architecture:
  ...
  4. Sub-agent edits the session notes file at ~/.cc_mini_agent/sessions/<cwd_hash>/session-notes.md
...
from cc_mini_agent.memory.session_prompts import (
...
class SessionPersistence:
    ...
    def _compute_session_dir(self) -> str:
        """Compute session directory: ~/.cc_mini_agent/sessions/<cwd_hash>/"""
        cwd_hash = hashlib.sha256(self.cwd.encode()).hexdigest()[:12]
        home = os.path.expanduser("~")
        return os.path.join(home, ".cc_mini_agent", "sessions", cwd_hash)
    ...
    def _estimate_token_count(self, messages: list) -> int:
        ...
        """
        from cc_mini_agent.providers.tokens import estimate_tokens_messages
        from cc_mini_agent.core.messages import messages_to_api
    ...
    async def extract(
        ...
        async with self._extract_lock:
            ...
            try:
                ...
                from cc_mini_agent.core.messages import messages_to_api
                from cc_mini_agent.tools.file_tools import FileReadTool, FileEditTool
                from cc_mini_agent.core.tool import ToolContext
                import os
...
def create_session_persistence_hook(
    ...
    """
    from cc_mini_agent.core.hooks import HookContext, HookResult
